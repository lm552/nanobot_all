...
class WebSocketChannel(BaseChannel):
    ...
    def _get_nanobot_root(self) -> Path:
        """Return the .nanobot data directory.

        Prefers workspace-relative ``.nanobot`` over the home-directory default
        so that custom workspaces (e.g. a project checkout) work correctly.
        """
        if self._session_manager:
            ws_root = self._session_manager.workspace / ".nanobot"
            if ws_root.is_dir():
                return ws_root
    ...
    def _create_user_workspace(self, role: str, user_id: str) -> None:
        ...
        root = self._get_nanobot_root()
        # Try workspace .nanobot first, then home .nanobot as fallback
        templates_dir = root / "templates" / role
        if not templates_dir.is_dir():
            home_templates = Path.home() / ".nanobot" / "templates" / role
            if home_templates.is_dir():
                templates_dir = home_templates
            else:
                logger.warning("Template directory not found for role '{}'", role)
                return

        users_dir = root / "users" / role / user_id
        if users_dir.exists():
            return
        ...
        (users_dir / "source" / "lesson-plans").mkdir(parents=True, exist_ok=True)
        logger.info(
            "Created workspace for user: {} (role={}), templates from {}",
            user_id, role, templates_dir,
        )
    ...
    def _resolve_source_dir(self, request: WsRequest) -> Path | Response:
        ...
        if not role or not user_id:
            return _http_error(400, "role and user_id are required")
        source_dir = self._get_nanobot_root() / "users" / role / user_id / "source"
    ...
    def _handle_sessions_list(self, request: WsRequest) -> Response:
        ...
        if role and user_id:
            sessions_dir = self._get_nanobot_root() / "users" / role / user_id / "sessions"
    ...
    def _handle_session_messages(self, request: WsRequest, key: str) -> Response:
        ...
        if role and user_id:
            ...
            user_sessions_dir = self._get_nanobot_root() / "users" / role / user_id / "sessions"
            from nanobot.session.manager import SessionManager
    ...
    async def _dispatch_envelope(
        ...
        if t == "message":
            ...
            msg_meta.update(conn_meta)
            # Merge client-supplied metadata (e.g. { _skill: "lesson-plan" })
            client_meta = envelope.get("meta")
            if isinstance(client_meta, dict):
                msg_meta.update(client_meta)
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        # Forward skill metadata so clients can distinguish skill outputs
        if msg.metadata.get("_skill"):
            payload["_skill"] = msg.metadata["_skill"]
        raw = json.dumps(payload, ensure_ascii=False)
