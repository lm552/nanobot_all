...
class ContextBuilder:
    ...
    def build_system_prompt(
        ...
        # Load explicitly requested skills (e.g. via _skill metadata from client)
        explicit_skills = [s for s in (skill_names or []) if s not in set(always_skills)]
        if explicit_skills:
            explicit_content = self.skills.load_skills_for_context(explicit_skills)
            if explicit_content:
                parts.append(f"# Active Skills\n\n{explicit_content}")

        skills_summary = self.skills.build_skills_summary(
            exclude=set(always_skills) | set(explicit_skills)
        )
        if skills_summary:
