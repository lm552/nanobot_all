...
class AgentLoop:
    ...
    def __init__(
        ...
        # Check home .nanobot first (default), then workspace-relative .nanobot
        # (custom workspace), so role templates are found regardless of setup.
        nanobot_home = Path.home() / ".nanobot"
        home_candidate = nanobot_home / "templates"
        ws_candidate = workspace / ".nanobot" / "templates"
        # Prefer whichever actually has the most role subdirectories (handles
        # the case where home .nanobot exists but lacks teacher templates).
        home_roles = (
            {p.name for p in home_candidate.iterdir() if p.is_dir()}
            if home_candidate.is_dir() else set()
        )
        ws_roles = (
            {p.name for p in ws_candidate.iterdir() if p.is_dir()}
            if ws_candidate.is_dir() else set()
        )
        if home_candidate.is_dir() and (not ws_roles or home_roles >= ws_roles):
            self._templates_dir = home_candidate
        elif ws_candidate.is_dir():
            self._templates_dir = ws_candidate
            logger.info("Using workspace .nanobot templates directory: {}", self._templates_dir)
    ...
    def _resolve_user_context(
        ...
        if not user_ws.exists():
            ...
            (user_ws / "source" / "lesson-plans").mkdir(parents=True, exist_ok=True)
        ...
        logger.info(
            "[agent] 加载角色智能体模板: role={}, user_id={}, template_dir={}, user_ws={}",
            role,
            user_id,
            template_dir,
            user_ws,
    ...
    async def _process_message_inner(
        ...
        # Resolve skill names from client-supplied _skill metadata
        skill_names = None
        skill_meta = msg.metadata.get("_skill") if msg.metadata else None
        if skill_meta == "lesson-plan":
            skill_names = ["lesson-plan"]

        pending_ask_id = pending_ask_user_id(history)
        if pending_ask_id:
            initial_messages = ask_user_tool_result_messages(
                self.context.build_system_prompt(skill_names=skill_names, channel=msg.channel),
                history,
                pending_ask_id,
                msg.content,
            )
