...
<nav class="top-nav">
  ...
  <div class="nav-center">
    <span class="nav-tab" @click="$router.push('/teacher/lesson-plan')">教案与活动</span>
    <span class="nav-tab active">作业批改</span>
    <span class="nav-tab" @click="$router.push('/teacher/analytics')">学情分析</span>
  </div>
  <div class="nav-right">
...
async function handleLogin() { const t = loginUserId.value.trim(); if (!loginRole.value || !t) return; loginError.value = ''; loginLoading.value = true; const u = { role: loginRole.value, userId: t }; login(u); loginLoading.value = false }
function toggleAuthMode() { authMode.value = authMode.value === 'login' ? 'register' : 'login'; loginError.value = '' }
function handleLogout() { authLogout(); loginUserId.value = ''; loginRole.value = 'teacher'; authMode.value = 'login' }
